# -*- coding: utf-8 -*-
"""Rotten -Tomatoes- Movieclips-YouTube-API-Scrapping
...
channel_info_ = get_channel_info_func(youtube, ChannelId)
channel_info_

def get_playlist_info_func(youtube, playlist_id):
...
PlaylistId = 'UU3gNmTGu-TTbFPpfSs5kNkg'


VidsIds = get_playlist_info_func(youtube, PlaylistId)
len(VidsIds)
...
def get_comments_info_func(youtube, vids_ids):
    ...
    for i in range(len(vids_ids)):
      try:
        request = youtube.commentThreads().list(part='snippet,replies', videoId=vids_ids[i])
        response = request.execute()

        for comment in response['items']:
          comments_info = {'Video_id' : vids_ids[i],'Comments' : comment['snippet']['topLevelComment']['snippet']['textOriginal']}
          all_comments.append(comments_info)

      except:
        error = f'failed to fetch data for video id : {vids_ids[i]}'
        print(error)
...
comments_df.to_csv('movieclips_raw_comments.csv')
files.download('movieclips_raw_comments.csv')
